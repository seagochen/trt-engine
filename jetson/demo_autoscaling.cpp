...
int main(int argc, char** argv) {
    ...
    // Load the video file
    StreamReader reader(args["video"], 640, 640, 30);

    // Create the FPS counter
    FPSCounter counter;

    while (true) {
        // Assuming 'frame' is the image you want to display
        cv::Mat frame;
        ...
        drawTextWithBackground(frame, 
                    fps_text, // Text to display
                    cv::Point(500, 50), // Left-top corner of the text
                    0.8, // Font scale
                    2, // Thickness
                    cv::Scalar(0, 0, 0)); // Background color        

        // Dynamically adjust the image size based on window size
        resizeAndDisplay(frame, windowName);

        // Exit the loop on pressing ESC or 'q'
