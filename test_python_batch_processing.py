...
def test_yolopose_batch():
    ...
    try:
        ...
        print(f"   ✓ 批处理成功，返回 {len(results)} 个结果")

        # 测试2：单张图像
        print("\n3. 测试单张图像批处理...")
        results = pipeline.infer_batch([images[0]])
        print(f"   ✓ 单张图像批处理成功，返回 {len(results)} 个结果")
...
def test_efficientnet_batch():
    ...
    try:
        ...
        if results:
            result = results[0]
            print(f"   结果结构: class_id={result['class_id']}, "
...
def main():
    """主函数"""
    print("\n" + "="*60)
    print("Python 批处理功能修复验证脚本")
    print("="*60)
    print("\n注意: 请确保以下文件存在:")
    print("  - ./build/libtrtengine_v2.so")
    print("  - ./models/yolov8n-pose.engine (或修改脚本中的路径)")
    print("  - ./models/efficientnet.engine (或修改脚本中的路径)")
    print()
    ...
    print("测试总结")
    print("="*60)
    print(f"YoloPose 批处理: {'✓ 通过' if yolo_success else '✗ 失败'}")
    print(f"EfficientNet 批处理: {'✓ 通过' if eff_success else '✗ 失败'}")
