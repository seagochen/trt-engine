...
from pyengine.font import common
from pyengine.font.common import Point, BGRColor
...
def draw_text(frame: np.ndarray,
    ...
    # 1) 预先测量尺寸
    w, h, baseline = common.measure_text(text, font_path, font_size, font_scale, thickness)
    text_rect = (x, y, w, h)  # 供自动取色与背景使用

    # 2) 自动前景色
    final_color: BGRColor = color if color is not None else common.auto_text_color(frame, top_left, (w, h))

    if font_path:
        # --- Pillow 路径 ---
        font = common.ensure_font(font_path, font_size)
        pil = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

        # 背景
        if bg_color is not None:
            rect = (x - bg_padding, y - bg_padding, x + w + bg_padding, y + h + bg_padding)
            pil = common.draw_bg_rect_pil(pil, rect, bg_color, bg_alpha)
        ...
        if bg_color is not None:
            ...
            common.draw_bg_rect_cv2(frame, lt, rb, bg_color, bg_alpha)
...
def draw_text_with_outline(frame: np.ndarray,
    ...
    x, y = top_left
    w, h, baseline = common.measure_text(text, font_path, font_size, font_scale, thickness)
    auto_color = common.auto_text_color(frame, top_left, (w, h))
