...
InferYoloV8Obj::~InferYoloV8Obj() {
    ...
    g_vec_output.clear();
    LOG_VERBOSE_TOPIC("InferYoloV8Obj", "deconstructor", "Local buffer released successfully.");
}
...
std::vector<Yolo> InferYoloV8Obj::postprocess(const int batchIdx, const float cls) {
    ...
    int results = sct_yolo_post_proc(cuda_device_ptr, g_vec_output, g_int_outputFeatures, g_int_outputSamples, cls);
    if (results < 0) {
